...
- 呼び出しは**製品をパラメータ**にする。`--product` が `products/<key>.yaml` の `components`、`libraries/*.yaml` の `supports.chips`、対象 chip の scenario から probe セット（**scan ＋ characterization ＋ 各 chip × 対応 library**）を計算する。現行実装は scan + library probe の導出までで、scenario runner の実装時に拡張する。
